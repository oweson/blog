...
import com.zyd.blog.controller.third_party_service.juhe.stock.api.StockApi;
...
public class StockServiceImpl implements StockService {
    ...
    public StockResult getStockListByType(Integer page, String type) {
        if("HK".equalsIgnoreCase(type)){
            // 香港股市列表
           return StockApi.getHKStockList(page);
        }else if("SH".equalsIgnoreCase(type)){
            // 沪股列表
            return StockApi.getSHStockList(page);
        }else if("SZ".equalsIgnoreCase(type)){
            // 深圳股市列表
            return StockApi.getSZStockList(page);
        }else if("USA".equalsIgnoreCase(type)){
            // 美国股市列表
